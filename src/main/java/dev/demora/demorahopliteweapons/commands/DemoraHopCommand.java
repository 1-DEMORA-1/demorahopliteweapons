...
public class DemoraHopCommand implements CommandExecutor, TabCompleter {
    ...
    private final List<String> weaponNames = Arrays.asList(
        ...
        "reaper_scythe", "villager_staff", "dragon_katana"
    );
    ...
    private ItemStack createWeapon(String weaponName) {
        return switch (weaponName) {
            case "sculk_crossbow" -> plugin.getSculkCrossbowManager().createSculkCrossbow();
            case "midas_sword" -> plugin.getMidasSwordManager().createMidasSword();
            case "mjolnir" -> plugin.getMjolnirManager().createMjolnir();
            case "shadow_blade" -> plugin.getShadowBladeManager().createShadowBlade();
            case "reaper_scythe" -> plugin.getReaperScytheManager().createReaperScythe();
            case "villager_staff" -> plugin.getVillagerStaffManager().createVillagerStaff();
            case "dragon_katana" -> plugin.getDragonKatanaManager().createDragonKatana();
            default -> null;
        };
    }
